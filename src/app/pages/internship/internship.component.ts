...
export class InternshipComponent implements OnInit {
  ...
  constructor(public internshipService: InternshipService,
    ...
    public activateRoute: ActivatedRoute, public seoService: SEOService) { }

  ngOnInit() {
    this.seoService.updateTitle("Internship");
    ...
    this.internshipService.listCategory(1)
      .subscribe((success: any) => {
        if (success.result) {
          ...
          this.category.filter((internshipCategory) => {
            if (this.categoryId == internshipCategory._id) {
              this.seoService.updateTitle("Internship - " + internshipCategory.categoryName);
            }
          });
        } else {
          alert('Something went wrong');
